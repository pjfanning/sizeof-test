...
class JavaSizeEstimator {

  private static final Logger logger = LoggerFactory.getLogger(JavaSizeEstimator.class);
  ...
  private static long sampleArray(final Object array, final SearchState state, final Random rand,
    ...
    for (int i = 0; i < ARRAY_SAMPLE_SIZE; i++) {
      ...
      if (obj != null) {
        size += JavaSizeEstimator.estimate(obj, state.visited);
      }
    }
    return size;
  }
