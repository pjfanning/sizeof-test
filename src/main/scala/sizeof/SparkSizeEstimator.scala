...
/**
 * A trait that allows a class to give [[SparkSizeEstimator]] more accurate size estimation.
 * When a class extends it, [[SparkSizeEstimator]] will query the `estimatedSize`, and use
 ...
 * org.apache.spark.util.collection.SizeTracker still uses [[SparkSizeEstimator]] to
 * estimate the size. However, a [[KnownSizeEstimation]] can provide a better estimation without
 * using [[SparkSizeEstimator]].
 */
private[sizeof] trait KnownSizeEstimation {
...
object SparkSizeEstimator {
  ...
  private val logger = LoggerFactory.getLogger(SparkSizeEstimator.getClass)
  ...
  private def visitArray(array: AnyRef, arrayClass: Class[_], state: SearchState): Unit = {
    ...
    if (elementClass.isPrimitive) {
      ...
      if (length <= ARRAY_SIZE_FOR_SAMPLING) {
        ...
        val drawn = mutable.HashSet[Int](2 * ARRAY_SAMPLE_SIZE)
  ...
  private def sampleArray(
    ...
    for (i <- 0 until ARRAY_SAMPLE_SIZE) {
      ...
      if (obj != null) {
        size += SparkSizeEstimator.estimate(obj, state.visited)
      }
    }
    size
  }
